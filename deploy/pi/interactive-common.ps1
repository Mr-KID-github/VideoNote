...
function Write-Success($message) {
    Write-Host "${ESC_GREEN}✅${ESC_RESET} $message"
}

function Write-Fail($message) {
    Write-Host "${ESC_RED}❌${ESC_RESET} $message" >&2
}

function Write-Warn($message) {
    Write-Host "${ESC_YELLOW}⚠️${ESC_RESET} $message"
}

function Write-Info($message) {
    Write-Host "${ESC_CYAN}ℹ️${ESC_RESET} $message"
}
...
function Test-SshConnection($host, $user, $port, $timeout = 5) {
    $remote = "$user@$host"
    $output = ssh -o "ConnectTimeout=$timeout" -o "BatchMode=yes" -p $port $remote "echo ok" 2>&1
...
function Save-LocalConfig($host, $user, $port) {
...
function Get-DockerRemoteStatus($host, $user, $port) {
    $remote = "$user@$host"
    $script = @'
...
'@
    $output = ssh -p $port $remote $script 2>&1
    $result = @{
